...
const prefix   = "#";
const category = "529601546204348427";
...
client.on('message',async message => {
    const emojis   = { yes: `${client.guilds.find(r => r.id === '520005460879867909').emojis.find(e => e.name === 'Yes')}`, wrong: `${client.guilds.find(r => r.id === '520005460879867909').emojis.find(e => e.name === 'Wrong')}` };
    ...
    if(args[0].toLowerCase() === `${prefix}help`) {
		...
		let openReason = "";
		current++;
    	message.guild.createChannel(`octicket-${current}`, 'text').then(c => {
		tchannels.push(c.id);
